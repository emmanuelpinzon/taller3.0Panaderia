		...
		case "ELIMINAR":
			...
			try {
				if (panDulce) {
					...
					vf.getCon()
							.mostrarAlerta(resultado ? "Producto eliminado correctamente" : "Producto no encontrado");
				}
				if (panQueso) {
					String listaProductos = mf.getPanQuesoDAO().ShowAllNames(); // Obtén la lista de productos
					vf.getCon().mostrarListado("Nombres disponibles:\n" + listaProductos);
		...
		case "AGREGARPRODUCTO":
			try {
				if (panDulce) {
					...
					mf.getPanDulceDAO().add(new PanDulceDTO(cantidadPan, nombre, peso, precio, tieneGluten,
							tieneLevadura, tieneArequipe, saborDulce));
					vf.getCon().mostrarMensajeEmergente("Producto Pan Dulce creado exitosamente");
				}

				if (panQueso) {
					String cantidad1 = vf.getPrincipal().getPanelEntrada().getCantidadPan().getText();
					ExceptionChecker.verificarNumeroNegativo(cantidad1);
					int cantidadPan1 = vf.getCon().readInt(cantidad1);

					String nombre1 = vf.getPrincipal().getPanelEntrada().getNombre().getText();
					ExceptionChecker.checkWord(nombre1);

					double peso1 = Double.parseDouble(vf.getPrincipal().getPanelEntrada().getPeso().getText());
					ExceptionChecker.checkDouble(peso1);
					double precio1 = Double.parseDouble(vf.getPrincipal().getPanelEntrada().getPrecio().getText());
					ExceptionChecker.checkDouble(precio1);
					String gluten1 = vf.getPrincipal().getPanelEntrada().getTieneGluten().getText();
					boolean tieneGluten = vf.getCon().readBoolean(gluten1);
					ExceptionChecker.checkCondition(gluten1);
					String levadura1 = vf.getPrincipal().getPanelEntrada().getTieneLevadura().getText();

					boolean tieneLevadura = vf.getCon().readBoolean(levadura1);
					ExceptionChecker.checkCondition(levadura1);
					...
					// Crear y agregar el producto
					mf.getPanQuesoDAO().add(new PanQuesoDTO(cantidadPan1, nombre1, peso1, precio1, tieneGluten,
							tieneLevadura, tipoQueso, esGratinado));
				...
				if (panIntegral) {
					...
					// Crear y agregar el producto
					mf.getPanIntegralDAO().add(new PanIntegralDTO(cantidadPan, nombre, peso, precio, tieneGluten,
							tieneLevadura, tieneSieteGranos, tieneFrutosSecos));
				...
				if (panHojaldrado) {
					...
					// Crear y agregar el producto
					mf.getPanHojaldradoDAO().add(new PanHojaldradoDTO(cantidadPan, nombre, peso, precio, tieneGluten,
							tieneLevadura, esCroissant, tieneCarne));
				...
				vf.getCon().mostrarAlerta("se ingresaron datos invalidos");
			}
		...
		case "UPDATE":
			try {
				if (panDulce == true && newPanDulce == true) {
					...
					String cantidad6 = vf.getPrincipal().getPanelEntrada().getCantidadPan().getText();
					ExceptionChecker.verificarNumeroNegativo(cantidad6);
					int cantidadPan = vf.getCon().readInt(cantidad6);

					String nombre = vf.getPrincipal().getPanelEntrada().getNombre().getText();
					ExceptionChecker.checkWord(nombre);

					double peso5 = Double.parseDouble(vf.getPrincipal().getPanelEntrada().getPeso().getText());

					ExceptionChecker.checkDouble(peso5);
					...
					PanDulceDTO nuevoPanDulce = new PanDulceDTO(cantidadPan, nombre, peso5, precio, tieneGluten,
							tieneLevadura, tieneArequipe, saborDulce);
				...
				if (panIntegral == true && newPanIntegral == true) {
					String cantidad2 = vf.getPrincipal().getPanelEntrada().getCantidadPan().getText();
					ExceptionChecker.verificarNumeroNegativo(cantidad2);
					int cantidadPan2 = vf.getCon().readInt(cantidad2);

					String nombre2 = vf.getPrincipal().getPanelEntrada().getNombre().getText();
					ExceptionChecker.checkWord(nombre2);

					double peso2 = Double.parseDouble(vf.getPrincipal().getPanelEntrada().getPeso().getText());

					ExceptionChecker.checkDouble(peso2);

					double precio2 = Double.parseDouble(vf.getPrincipal().getPanelEntrada().getPrecio().getText());
					ExceptionChecker.checkDouble(precio2);
					String gluten = vf.getPrincipal().getPanelEntrada().getTieneGluten().getText();
				...
				if (panHojaldrado == true && newPanHojaldrado == true) {
					...
					if (anteriorPanHojaldrado != null) {
						// Llama al método update del DAO con los objetos anterior y nuevo
						boolean actualizado = mf.getPanHojaldradoDAO().update(anteriorPanHojaldrado,
								nuevoPanHojaldrado);
				...
				vf.getCon().mostrarAlerta(e2.getMessage());
			} catch (InvalidBooleanStateException e3) {
				vf.getCon().mostrarAlerta(e3.getMessage());
